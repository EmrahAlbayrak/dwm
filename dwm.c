...
#define INTERSECT(x,y,w,h,m)    (MAX(0, MIN((x)+(w),(m)->wx+(m)->ww) \
        - MAX((x),(m)->wx)) * MAX(0, MIN((y)+(h),(m)->wy+(m)->wh) \
        - MAX((y),(m)->wy)))
...
/* color schemes */
enum { SchemeNorm, SchemeSel, SchemeInd, SchemeTitle,SchemeBtn };
/* EWMH atoms */
...
/* clicks */
enum { ClkTagBar, ClkLtSymbol, ClkStatusText, ClkMenu, ClkWinTitle,
       ClkClientWin, ClkRootWin, ClkLast, ClkCloseClient
     };
...
buttonpress(XEvent *e)
{
    ...
    if (ev->window == selmon->barwin)
    {
        ...
        if (ev->x < (x = TEXTW(iconMenu)))
            click = ClkMenu;
        else if (ev->x < (x += TEXTW(m->ltsymbol)))
            click = ClkLtSymbol;
        else
        {
            int wClose, wStatus, wTags, lTag, rTag;
            i = 0;
            wTags = 0;
            ...
            wClose = TEXTW(iconClose);
            wStatus = TEXTW(stext);
            lTag = TEXTW(iconMenu) + TEXTW(m->ltsymbol);
            rTag = lTag + wTags;
            if (ev->x > lTag && ev->x < rTag)
            {
                click = ClkTagBar;
                arg.ui = 1 << i;
            }
            else if (ev->x > selmon->ww - wStatus)
                click = ClkStatusText;
            else if (ev->x > selmon->ww - wStatus - wClose)
                click = ClkCloseClient;
            else
                click = ClkWinTitle;
        }
...
drawbar(Monitor *m)
{
    ...
    w = blw = TEXTW(iconClose);
    drw_text(drw, m->ww - sw - w, 0, w, bh, lrpad / 2, iconClose, 0);

    int wLeft = 0;
    x = 0;
    wLeft += w = blw = TEXTW(iconMenu);
    drw_setscheme(drw, scheme[SchemeNorm]);
    x = drw_text(drw, x, 0, w, bh, lrpad / 2, iconMenu, 0);

    wLeft += w = blw = TEXTW(m->ltsymbol);
    drw_setscheme(drw, scheme[SchemeNorm]);
    ...
    for (i = 0; i < LENGTH(tags); i++)
    {
        ...
        wTags += w = TEXTW(tags[i]);
        drw_setscheme(drw,
              scheme[m->tagset[m->seltags] & 1 << i ? SchemeSel : SchemeNorm]);
        drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);

        if(hInd > 0 && m->tagset[m->seltags] & 1 << i)
        {
            iw = w;
            drw_setscheme(drw, scheme[SchemeInd]);
            drw_rect(drw, x, drw->fonts->h + vertpadbar - hInd - 1,
                     iw, hInd, hInd, urg & 1 << i);
        }
        x += w;
    }
    ...
    if ((w = m->ww - sw - x) > bh)
    {
        if (m->sel)
        {
            ...
            int wName = TEXTW(m->sel->name);
            int lPad = (m->ww - wStatus - wName - wTags - wLeft) / 2;
        /* make sure name will not overlap on tags even when it is very long */
            lPad = lPad >= 10 ? lPad : 10;
            drw_text(drw, x, 0, w - TEXTW(iconClose), bh,
                    lPad, m->sel->name, 0);
...
incnmaster(const Arg *arg)
{
    ...
    if (arg->i > 0 && n == selmon->nmaster) return;
...
fake_signal(void)
{
    ...
    if (gettextprop(root, XA_WM_NAME, fsignal, sizeof(fsignal)))
    {
        ...
        if (len_indicator > len_fsignal ? 0 : strncmp(indicator, fsignal,
                len_indicator) == 0)
        {
...
manage(Window w, XWindowAttributes *wa)
{
    ...
    c->y = MAX(c->y, ((c->mon->by == c->mon->my)
            && (c->x + (c->w / 2) >= c->mon->wx)
            && (c->x +
                (c->w / 2) < c->mon->wx + c->mon->ww)) ? bh : c->mon->my);
    c->bw = borderpx;
...
movemouse(const Arg *arg)
{
    ...
    if (XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync,
            GrabModeAsync, None, cursor[CurMove]->cursor, CurrentTime)
            != GrabSuccess)
        return;
    if (!getrootptr(&x, &y))
        return;
...
runAutostart(void)
{
    ...
    if(system("cd ~/.config/scripts; ./autostart.sh &")) ;
}
...
setlayout(const Arg *arg)
{
    ...
    if (arg && arg->v)
        selmon->lt[selmon->sellt] = selmon->pertag->ltidxs
            [selmon->pertag->curtag][selmon->sellt] = (Layout *)arg->v;
...
tile(Monitor *m)
{
    ...
    if (n > m->nmaster)
        ...
        mw = m->ww - m->gappx;
    for (i = 0, my = ty = m->gappx, c = nexttiled(m->clients);
...
xerror(Display *dpy, XErrorEvent *ee)
{
    if (ee->error_code == BadWindow
        ...
        || (ee->request_code
            == X_PolySegment && ee->error_code == BadDrawable)
        || (ee->request_code
            == X_ConfigureWindow && ee->error_code == BadMatch)
        || (ee->request_code
            == X_GrabButton && ee->error_code == BadAccess)
